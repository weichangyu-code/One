...
static inline char* duplicateStringValue(const char* value,
  ...
  if (!(newString != 0))
	  JSON_FAIL_MESSAGE("in Json::Value::duplicateStringValue(): "
	                    "Failed to allocate string value buffer");
...
/*  JSON_ASSERT_MESSAGE(
  ...
  if (!(text[0] == '\0' || text[0] == '/'))
  	JSON_FAIL_MESSAGE("in Json::Value::setComment(): Comments must start with /");
...
Value
Value::removeMember(ArrayIndex index)
{
	...
	return old;
#else
	Value *value = value_.map_->find( index );
	if (value){
		Value old(*value);
		value_.map_.remove( index );
		return old;
	} else {
		return null;
	}
